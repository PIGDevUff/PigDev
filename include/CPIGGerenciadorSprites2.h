...
public:
    ...
    static PIGSprite GetSprite(int idSprite){
        //std::unordered_map<int,PIGSprite>::iterator it = sprites.find(idSprite);
        ...
        PIGSprite sp = sprites[idSprite];
        auto end = std::chrono::high_resolution_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin);
        if (cont++<200000) fprintf(arq,"%d\n",elapsed.count());
        else exit(-1);
        return sp;
    }
    ...
    static void Encerra(){
        ...
        //for(std::unordered_map<int,PIGSprite>::iterator it = sprites.begin(); it != sprites.end(); ++it) {
        //    delete it->second;
        //}
    }
