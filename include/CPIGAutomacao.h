...
public:
    ...
    CPIGAutomacao(int idProprietario){
        ...
        transAtual = 0;
        somaTrans = +1;
        timerAcoes = pigGerTimers.CriaTimer(true);//new CPIGTimer(true);
        iniciado = false;
    }
    ...
    CPIGAutomacao(int idProprietario, CPIGAutomacao *outro){
        ...
        timerAcoes = pigGerTimers.CriaTimer(true);
        timelineAcoes = outro->timelineAcoes;
        for (PIGTransicao t: outro->transicoes){
            transicoes.push_back(new CPIGTransicao(t));
        }
        iniciado = false;
    }
    ...
    virtual ~CPIGAutomacao(){
        //delete timerAcoes;
        pigGerTimers.Remove(timerAcoes->GetID());
        LimpaTransicoes();
        timelineAcoes.clear();
    }
